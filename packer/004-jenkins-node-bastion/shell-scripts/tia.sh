...
## Setup Ansible and Docker User
sudo useradd jenkins -m -d /home/jenkins -s /bin/bash
sudo useradd ansible -m -d /home/ansible -s /bin/bash
sudo echo -e 'jenkins  ALL=(ALL)  NOPASSWD:  ALL' > /etc/sudoers.d/jenkins
sudo echo -e 'ansible  ALL=(ALL)  NOPASSWD:  ALL' > /etc/sudoers.d/ansible
sudo SocketGroup=docker
sudo systemctl daemon-reload
sudo chmod 666 /var/run/docker.sock
sudo usermod jenkins -aG docker
sudo usermod ansible -aG docker
